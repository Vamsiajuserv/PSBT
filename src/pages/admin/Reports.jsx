...
import {
  ...
  Landmark, Gavel, UtensilsCrossed, Recycle, FileBarChart, CalendarCheck,
} from 'lucide-react'
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-5 mb-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 items-end">
        <div>
          ...
          <div className="flex items-center gap-1.5">
            <DateField value={start} onChange={(e) => setStart(e.target.value)} className="input !px-2.5 text-[0.78125rem] flex-1 min-w-0" />
            <span className="text-gray-400 shrink-0">–</span>
            <DateField value={end} onChange={(e) => setEnd(e.target.value)} className="input !px-2.5 text-[0.78125rem] flex-1 min-w-0" />
            <button onClick={setToday} className="shrink-0 h-[38px] px-2 rounded-lg border border-gray-200 bg-white text-[0.7rem] font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors flex items-center gap-1" title={tr("Today")}><CalendarCheck size={13} /><span className="hidden sm:inline"><T>Today</T></span></button>
          </div>
        </div>
        <div>
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button onClick={() => generate(report, start, end)} className="btn-maroon !py-2.5"><Search size={14} />{' '}<T>Generate Report</T></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden" id="print-area">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {!loading && !loadErr && result?.columns?.length > 0 && (result.rows || []).map((row, i) => (
                ...
                {!loading && !loadErr && result && result.rows?.length === 0 && <tr><td colSpan={result.columns?.length || 1} className="px-5 py-12 text-center text-gray-400"><T>No records for the selected period.</T></td></tr>}
          ...
          {result?.rows && <div className="px-5 py-3.5 border-t border-gray-100 text-[0.8125rem] text-gray-500">{tr('Showing')} 1 {tr('to')} {result.rows.length} {tr('of')} {result.rows.length} {tr('records')}</div>}
        </div>
      </div>
    </div>
  )
