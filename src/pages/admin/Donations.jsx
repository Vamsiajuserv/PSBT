...
export default function Donations() {
  ...
  return (
    <div>
      ...
      {drawer && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <form onSubmit={(e) => save(e, true)} className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="px-6 py-4 border-t border-gray-100 sticky bottom-0 bg-white space-y-3">
              {panErr && !panErr.includes('PAN') && <div className="text-sm text-red-600 bg-red-50 border border-red-100 px-3 py-2 rounded-lg">{panErr}</div>}
              <div className="flex gap-3">
                ...
                <button type="submit" disabled={saving} className="btn-maroon flex-1 justify-center disabled:opacity-60">{saving ? tr('Saving…') : <>{tr('Save & Print Receipt')} <Printer size={14} /></>}</button>
              </div>
            </div>
          </form>
        </div>
      ...
      {console.log('printDoc state:', printDoc)}
      {printDoc && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={() => setPrintDoc(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{background:'white',borderRadius:'12px',padding:'24px',maxWidth:'400px',width:'90%',boxShadow:'0 10px 40px rgba(0,0,0,0.3)'}}>
            <div style={{textAlign:'center',borderBottom:'2px solid #8b4513',paddingBottom:'16px',marginBottom:'16px'}}>
              <div style={{fontSize:'48px'}}>🛕</div>
              <h1 style={{fontSize:'20px',fontWeight:'bold',color:'#8b4513',margin:'8px 0'}}>Sri Shirdi Sai Baba Temple</h1>
              <p style={{color:'#666',fontSize:'14px'}}>Donation Receipt</p>
            </div>
            <div style={{background:'#8b4513',color:'white',textAlign:'center',padding:'10px',borderRadius:'6px',marginBottom:'16px',fontWeight:'bold',letterSpacing:'2px'}}>
              DONATION RECEIPT
            </div>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:'16px',fontSize:'14px'}}>
              <span><strong>Receipt No:</strong> {printDoc.receipt_no || 'N/A'}</span>
              <span><strong>Date:</strong> {printDoc.donated_on || printDoc.created_at ? fmtDate(printDoc.donated_on || printDoc.created_at) : 'N/A'}</span>
            </div>
            <div style={{background:'#f9f9f9',border:'1px solid #ddd',borderRadius:'8px',padding:'16px',marginBottom:'16px'}}>
              <div style={{display:'flex',justifyContent:'space-between',borderBottom:'1px dashed #ccc',paddingBottom:'8px',marginBottom:'8px'}}>
                <span style={{color:'#666'}}>Donor</span>
                <span style={{fontWeight:'600'}}>{printDoc.donor_name || 'Anonymous'}</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',borderBottom:'1px dashed #ccc',paddingBottom:'8px',marginBottom:'8px'}}>
                <span style={{color:'#666'}}>Category</span>
                <span style={{fontWeight:'600'}}>{printDoc.fund || 'N/A'}</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',borderBottom:'1px dashed #ccc',paddingBottom:'8px',marginBottom:'8px'}}>
                <span style={{color:'#666'}}>Type</span>
                <span style={{fontWeight:'600'}}>{TYPE_LABEL[printDoc.donation_type] || printDoc.donation_type || 'N/A'}</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',borderBottom:'1px dashed #ccc',paddingBottom:'8px',marginBottom:'8px'}}>
                <span style={{color:'#666'}}>Payment Mode</span>
                <span style={{fontWeight:'600'}}>{printDoc.mode || 'N/A'}</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between'}}>
                <span style={{color:'#666'}}>Tax Exemption</span>
                <span style={{fontWeight:'600'}}>{printDoc.g80 ? 'Eligible (80G)' : '—'}</span>
              </div>
            </div>
            <div style={{background:'#fffbeb',border:'2px solid #d97706',borderRadius:'8px',padding:'16px',textAlign:'center',marginBottom:'16px'}}>
              <div style={{color:'#666',fontSize:'14px'}}>Total Amount</div>
              <div style={{fontSize:'32px',fontWeight:'bold',color:'#8b4513'}}>₹ {Number(printDoc.amount || 0).toLocaleString('en-IN')}</div>
            </div>
            <div style={{textAlign:'center',color:'#8b4513',fontWeight:'600',marginBottom:'8px'}}>|| Om Sri Sai Ram ||</div>
            <p style={{textAlign:'center',fontSize:'11px',color:'#999'}}>This is a computer-generated receipt.</p>
            <div style={{display:'flex',gap:'12px',justifyContent:'center',marginTop:'20px',paddingTop:'16px',borderTop:'1px solid #eee'}}>
              <button onClick={() => window.print()} className="btn-maroon"><Printer size={15} /> Print</button>
