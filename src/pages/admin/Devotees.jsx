              ...
              {rows.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50/60">
                  ...
                  <td className="px-4 py-3.5 flex gap-1">
                    ...
                    {isAdmin && <button onClick={() => { setSaveErr(''); setModal({ mode: 'edit', data: { ...d } }) }} title={tr("Edit devotee")} className="w-8 h-8 grid place-items-center rounded-lg border border-gray-200 text-gray-400 hover:text-maroon-700 hover:border-maroon-300"><Pencil size={15} /></button>}
                  </td>
                </tr>
              ))}
          ...
          <form onClick={(e) => e.stopPropagation()} onSubmit={save} className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="input" value={modal.data.name_te || ''}
                  onChange={(e) => setModal({ ...modal, data: { ...modal.data, name_te: e.target.value } })} />
              </div>
              <div>
              ...
              <div>
                ...
                <input className="input" value={modal.data.city || ''} onChange={(e) => setModal({ ...modal, data: { ...modal.data, city: e.target.value } })} />
              </div>
              <div>
