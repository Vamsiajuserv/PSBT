...
import { Users, UserCog, ShieldCheck, Pencil, X, IndianRupee, Flame, Search, RotateCcw, Plus, Trash2, Save } from 'lucide-react'
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/70 text-left text-[0.6875rem] uppercase tracking-wide text-gray-500">
                {[tr('Member ID'), tr('Name'), tr('Designation'), tr('Phone'), tr('Email'), tr('Status'), tr('Actions')].map((c) => (
                  ...
                  <td className="px-4 py-3.5">
                    {canWrite && (
                      <div className="flex gap-1">
                        <button onClick={() => { setErr(''); setFieldErrors({}); setDrawer({ mode: 'edit', data: { ...r } }) }} title={tr("Edit")} className="w-8 h-8 grid place-items-center rounded-lg border border-gray-200 text-gray-400 hover:text-maroon-700 hover:border-maroon-300"><Pencil size={15} /></button>
                        <button onClick={() => remove(r)} title={tr("Delete")} className="w-8 h-8 grid place-items-center rounded-lg border border-gray-200 text-gray-400 hover:text-red-600 hover:border-red-300"><Trash2 size={15} /></button>
                      </div>
                    )}
                  </td>
                </tr>
              ))}
              {items.length === 0 && <TableStates colSpan={7} loading={loading} error={loadErr} onRetry={load} empty={tr("No committee members found.")} />}
            </tbody>
          </table>
        </div>
      </div>
          ...
          <form onClick={(e) => e.stopPropagation()} onSubmit={save} className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="flex-1 px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input className="input" value={drawer.data.name_te || ''} onChange={(e) => setDrawer({ ...drawer, data: { ...drawer.data, name_te: e.target.value } })} />
              </div>
              <div>
