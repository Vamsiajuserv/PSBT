...
export default function About() {
  ...
  return (
    <div className="bg-cream">
      <MinimalBanner title={tr("About Our Temple")} breadcrumb="Home  ›  About Temple" />
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 grid lg:grid-cols-[300px_1fr_300px] gap-8">
        ...
        {/* Narrative */}
        <div>
          <div className="font-script text-3xl text-gold-500 leading-none"><T>Sai Sharanam</T></div>
          ...
          <div className="bg-gold-50 border-l-4 border-gold-400 rounded-r-xl px-5 py-4 mt-6">
            ...
            <p className="text-xs text-black mt-1"><T>— Shri Sai Baba</T></p>
          </div>
          <div className="flex flex-wrap gap-3 mt-7">
            <Link to="/history" className="btn-outline"><T>Temple History</T>{' '}<ArrowRight size={15} /></Link>
            <Link to="/festivals" className="btn-primary"><T>Festivals & Events</T>{' '}<ArrowRight size={15} /></Link>
          </div>
        </div>

        {/* Info card */}
        <div className="card p-5 h-fit">
          <div className="font-display text-xs uppercase tracking-widest text-maroon-700"><T>Temple Information</T></div>
            ...
            {INFO.map((it) => {
              ...
              return (
                <div key={it.label} className="flex gap-3">
                  ...
                  <div>
                    <div className="text-[0.6875rem] font-bold uppercase tracking-wide text-gold-600"><T>{it.label}</T></div>
          ...
          {PRINCIPLES.map((p) => {
            ...
            return (
              <div key={p.title} className="card p-5 text-center hover:-translate-y-1 transition-transform">
                ...
                <h3 className="font-bold text-maroon-700 mt-3"><T>{p.title}</T></h3>
                <p className="text-[0.6875rem] text-black mt-1.5 leading-relaxed"><T>{p.desc}</T></p>
              </div>
            )
          })}
        </div>
