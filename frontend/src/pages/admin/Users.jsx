    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-5 flex flex-col lg:flex-row lg:items-end gap-4">
          ...
          <div><label className="block text-[0.75rem] text-gray-500 mb-1.5">Select Role</label><Select value={role} onChange={(e) => setRole(e.target.value)} className="input !w-48"><option value="">All Roles</option>{roles.map((r) => <option key={r}>{r}</option>)}</Select></div>
          <div><label className="block text-[0.75rem] text-gray-500 mb-1.5">Status</label><Select value={status} onChange={(e) => setStatus(e.target.value)} className="input !w-40"><option value="">All Status</option><option>Active</option><option>Inactive</option></Select></div>
                ...
                <tr key={u.id} className="hover:bg-gray-50/60">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      {u.role === 'Administrator' && <span className="text-[0.625rem] font-semibold text-maroon-700 bg-maroon-50 rounded px-1.5 py-0.5">Admin</span>}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 relative">
                      ...
                      }} title="Show 2FA setup secret" className="px-2 h-8 rounded-lg border border-amber-200 text-amber-700 text-[0.6875rem] font-bold hover:bg-amber-50">2FA</button>}
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <form onSubmit={save} className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="px-6 py-5 space-y-4 flex-1">
              ...
              {tab === 'access' && (
                <>
                  <p className="text-[0.78125rem] text-gray-500">Module access for this user. Defaults follow the selected role — customise as needed.</p>
            ...
            <div className="px-6 py-4 border-t border-gray-100 sticky bottom-0 bg-white">
              ...
              <div className="mt-3 flex items-start gap-2 text-[0.75rem] text-gray-600 bg-blue-50/70 border border-blue-100 rounded-lg px-3 py-2.5"><Info size={14} className="text-blue-500 shrink-0 mt-0.5" /> User will receive login credentials on their registered email.</div>
            </div>
          </form>
        </div>
      )}
