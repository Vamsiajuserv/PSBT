...
export default function DonationMaster() {
  ...
  return (
    <div>
      <PageTitle title="Donation Master" subtitle="Maintain and configure donation categories used for cash donations, material donations and sponsorships."
        actions={canWrite && <button onClick={() => setDrawer({ mode: 'create', data: emptyCat() })} className="btn-maroon !py-2.5"><Plus size={16} /> Add New Category</button>} />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatTile icon={HandHeart} color="#059669" bg="bg-emerald-50" title="Total Categories" value={stats ? num(stats.total) : '—'} sub="Active Donation Categories" />
        <StatTile icon={Coins} color="#d97706" bg="bg-amber-50" title="Cash Categories" value={stats ? num(stats.cash) : '—'} sub="Cash Donation Categories" />
        <StatTile icon={Package} color="#2563eb" bg="bg-blue-50" title="Material Categories" value={stats ? num(stats.material) : '—'} sub="Material Donation Categories" />
        <StatTile icon={Users} color="#7c3aed" bg="bg-violet-50" title="Sponsorship Categories" value={stats ? num(stats.sponsorship) : '—'} sub="Sponsorship Categories" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={c.id} className="hover:bg-gray-50/60">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      {canWrite && <button onClick={() => setDrawer({ mode: 'edit', data: { ...c } })} title="Edit" className="w-8 h-8 grid place-items-center rounded-lg border border-gray-200 text-maroon-600 hover:bg-maroon-50"><Pencil size={15} /></button>}
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="px-5 py-12 text-center text-gray-400">No categories found.</td></tr>}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <form onSubmit={save} className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 sticky bottom-0 bg-white">
              ...
              <button className="btn-maroon flex-1 justify-center"><Save size={15} /> Save Category</button>
            </div>
          </form>
        </div>
      )}
