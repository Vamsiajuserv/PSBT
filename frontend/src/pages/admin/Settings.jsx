  ...
  },
  {
    ...
    subs: [{ key: 'login', label: 'Login Policy', subtitle: 'Manage login and security preferences.',
      fields: [{ k: 'session_timeout_minutes', label: 'Session Timeout (minutes)' }, { k: 'max_login_attempts', label: 'Max Login Attempts' },
        { k: 'enforce_2fa', label: 'Enforce Two-Factor Auth', type: 'select', options: ['No', 'Yes'] }] }],
  },
  {
...
export default function Settings() {
  ...
  return (
    <div>
      ...
      {/* Settings Categories */}
      <div className="text-[15px] font-bold text-maroon-800 mb-3">Settings Categories</div>
        ...
        {CATS.map((c) => {
          ...
          return (
            <button key={c.key} onClick={() => selectCat(c)}
              ...
              <div className="text-[13px] font-bold text-gray-800 mt-2.5">{c.title}</div>
              <div className="text-[11.5px] text-gray-400 mt-1 leading-snug">{c.desc}</div>
            </button>
          )
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-5">
            ...
            return (
              <div key={c.key}>
                <button onClick={() => { setOpen((o) => ({ ...o, [c.key]: !isOpen })); if (!isOpen) selectCat(c) }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13.5px] font-semibold ${cat === c.key ? 'text-maroon-700' : 'text-gray-600 hover:bg-gray-50'}`}>
                  ...
                  <div className="ml-3 pl-3 border-l border-gray-100 mb-1 space-y-0.5">
                    {c.subs.map((s) => (
                      <button key={s.key} onClick={() => { setCat(c.key); setSub(s.key) }}
                        className={`w-full flex items-center gap-2 text-left px-3 py-2 rounded-lg text-[13px] transition-colors ${cat === c.key && sub === s.key ? 'bg-maroon-50 text-maroon-700 font-semibold' : 'text-gray-500 hover:bg-gray-50'}`}>
        ...
        <div className="space-y-5">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
              {section.fields.map((f) => (
                <div key={f.k} className={f.full || f.type === 'textarea' ? 'sm:col-span-2 lg:col-span-3' : ''}>
                  <label className="block text-[12.5px] font-medium text-gray-600 mb-1.5">{f.label} {f.req && <span className="text-red-500">*</span>}</label>
                  {f.type === 'textarea' ? (
                    <>
                      <textarea className="input min-h-[80px]" maxLength={f.max} value={data[f.k] || ''} onChange={(e) => set(f.k, e.target.value)} />
                      {f.max && <div className="text-right text-[11px] text-gray-400 mt-0.5">{(data[f.k] || '').length} / {f.max}</div>}
                    </>
                  ) : f.type === 'select' ? (
            ...
            {section.logos && (
              <div className="grid sm:grid-cols-2 gap-6 mt-7 pt-6 border-t border-gray-100">
                {[['temple_logo', 'Temple Logo', 'This logo will be used in the system and reports.'],
                  ...
                  <div key={k}>
                    <div className="text-[13px] font-bold text-maroon-700">{title}</div>
                    <div className="text-[12px] text-gray-400 mb-3">{note}</div>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-[12px] text-gray-500">Current Logo</div>
                        <div className="text-[13px] font-medium text-gray-700 mb-1.5">{data[k] || '—'}</div>
                        <label className="btn-outline !py-1.5 cursor-pointer inline-flex text-[12px]"><Upload size={13} /> Change Logo
                          <input type="file" accept="image/png,image/jpeg" className="hidden" onChange={(e) => set(k, e.target.files?.[0]?.name || data[k])} /></label>
                        <div className="text-[11px] text-gray-400 mt-1">Allowed: PNG, JPG (Max 2MB)</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-gray-50/70 rounded-xl border border-gray-100 px-5 py-4">
            <div className="flex items-center gap-2 text-maroon-700 font-semibold text-[13.5px] mb-3"><Info size={15} /> Audit Information</div>
          ...
          <div className="flex items-center gap-2 text-[13px] text-gray-500 bg-blue-50/60 border border-blue-100 rounded-lg px-4 py-2.5">
...
function Meta({ label, value }) {
  return (
    <div>
      ...
      <div className="text-[13px] text-gray-800 font-medium mt-0.5">{value || '—'}</div>
    </div>
  )
}
