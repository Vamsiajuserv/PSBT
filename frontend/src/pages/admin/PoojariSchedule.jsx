      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex items-center gap-6 px-5 border-b border-gray-100">
          {[['list', 'List View', List], ['calendar', 'Calendar View', CalendarDays]].map(([k, label, Icon]) => (
            <button key={k} onClick={() => setTab(k)} className={`flex items-center gap-2 py-3.5 text-[13.5px] font-semibold border-b-2 -mb-px ${tab === k ? 'border-maroon-600 text-maroon-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
              <Icon size={15} /> {label}
            </button>
          ))}
        </div>
          ...
          <>
            <div className="px-5 py-5">
              <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <div><label className="block text-[12px] text-gray-500 mb-1.5">Search by Poojari or Pooja Name</label>
                ...
                <div><label className="block text-[12px] text-gray-500 mb-1.5">Date Range</label>
                  <div className="flex items-center gap-1"><input type="date" value={start} onChange={(e) => setStart(e.target.value)} className="input !px-2 !text-[12px]" /><span className="text-gray-300">–</span><input type="date" value={end} onChange={(e) => setEnd(e.target.value)} className="input !px-2 !text-[12px]" /></div></div>
                <div><label className="block text-[12px] text-gray-500 mb-1.5">Pooja</label>
                ...
                <div><label className="block text-[12px] text-gray-500 mb-1.5">Poojari</label>
                ...
                <div><label className="block text-[12px] text-gray-500 mb-1.5">Schedule Status</label>
                ...
                <tbody className="divide-y divide-gray-100">
                  {rows.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-50/60">
                      ...
                      <td className="px-5 py-3.5 text-[13px] text-gray-600 whitespace-nowrap">{fmtDate(s.schedule_date)}<span className="block text-[11px] text-gray-400">{weekday(s.schedule_date)}</span></td>
                      <td className="px-5 py-3.5 text-[13px] text-gray-600 whitespace-nowrap">{s.start_time} –<span className="block">{s.end_time}</span></td>
                      <td className="px-5 py-3.5 text-gray-600 text-[13px]">{s.execution_frequency}</td>
            ...
            <div className="px-5 py-3.5 border-t border-gray-100 flex items-center justify-between">
              <span className="text-[13px] text-gray-500">Showing {total === 0 ? 0 : (page - 1) * SIZE + 1} to {Math.min(page * SIZE, total)} of {total} schedules</span>
              <div className="flex items-center gap-1.5">
                ...
                {Array.from({ length: Math.min(pageCount, 4) }, (_, i) => i + 1).map((n) => (
                  <button key={n} onClick={() => setPage(n)} className={`w-8 h-8 grid place-items-center rounded-lg text-[13px] font-semibold ${n === page ? 'bg-maroon-700 text-cream' : 'border border-gray-200 text-gray-600'}`}>{n}</button>
                ))}
          ...
          <form onSubmit={save} className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="px-6 py-5 border-b border-gray-100 flex items-start justify-between">
              <div><h3 className="font-serif text-xl font-bold text-maroon-800">{drawer.id ? 'Edit Schedule' : 'Assign Poojari Schedule'}</h3>
                <p className="text-[13px] text-gray-500 mt-0.5">Assign poojari to a pooja for specific date and time.</p></div>
              <button type="button" onClick={() => setDrawer(null)} className="text-gray-400 hover:text-maroon-700"><X size={20} /></button>
            </div>

            <div className="px-6 py-5 space-y-5 flex-1">
              <div className="text-[13px] font-bold text-maroon-700">1. Assignment Details</div>
              ...
              <div><label className="label">Schedule Date *</label><input required type="date" className="input" value={drawer.data.schedule_date} onChange={(e) => setDrawer({ ...drawer, data: { ...drawer.data, schedule_date: e.target.value } })} /></div>
              ...
              <div className="text-[13px] font-bold text-maroon-700 pt-1">2. Additional Information</div>
              <div><label className="label">Notes (Optional)</label><textarea className="input min-h-[72px]" placeholder="Enter any notes or special instructions…" value={drawer.data.notes} onChange={(e) => setDrawer({ ...drawer, data: { ...drawer.data, notes: e.target.value } })} /></div>
              <div className="bg-amber-50/60 border border-amber-100 rounded-lg px-3 py-2.5 text-[12px] text-gray-600 flex items-start gap-2"><Info size={15} className="text-amber-500 shrink-0 mt-0.5" /> Selecting a Pooja will load only the plans configured in Pooja Master.</div>
            </div>
        ...
        {dates.map((d) => (
          <div key={d} className="border border-gray-100 rounded-xl p-4">
            ...
            <div className="space-y-2">
              {byDate[d].map((s) => (
                <div key={s.id} className="flex items-center gap-2 text-[13px] border-b border-dashed border-gray-100 pb-2">
                  <span className="text-gray-400 text-[11px] w-16">{s.start_time}</span>
                  <span className="flex-1"><span className="font-semibold text-gray-800">{s.pooja_name}</span><span className="block text-[11px] text-gray-400">{s.poojari_name || 'Unassigned'}</span></span>
