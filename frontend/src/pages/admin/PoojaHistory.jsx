...
export default function PoojaHistory() {
  ...
  return (
    <div>
      <PageTitle title="Pooja History" subtitle="View completed and historical pooja records." />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatTile icon={Flame} color="#ea580c" bg="bg-orange-50" title="Total Completed Poojas"
          value={stats ? num(stats.total_completed) : '—'} sub="All time completed poojas" />
        <StatTile icon={CalendarCheck} color="#059669" bg="bg-emerald-50" title="Completed This Month"
          value={stats ? num(stats.completed_this_month) : '—'} sub={`Poojas completed in ${monthLabel()}`} />
        <StatTile icon={Users} color="#d97706" bg="bg-amber-50" title="Devotees Served"
          value={stats ? num(stats.devotees_served) : '—'} sub="Unique devotees served" />
        <StatTile icon={InfinityIcon} color="#7c3aed" bg="bg-violet-50" title="Active Long-Term Poojas"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {rows.length === 0 && <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-400">No pooja records found.</td></tr>}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl flex flex-col">
            ...
            <div className="px-6 py-5 space-y-6 flex-1">
              <DSection icon={FileText} n="1" title="Booking Information">
              ...
              <DSection icon={User} n="2" title="Devotee Information">
              ...
              <DSection icon={Sparkles} n="3" title="Pooja & Plan Details">
              ...
              <DSection icon={ClipboardList} n="4" title="Poojari & Execution Details">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 sticky bottom-0 bg-white">
              <button onClick={() => setPrintDoc(drawer)} className="btn-outline flex-1 justify-center"><Printer size={15} /> Print Receipt</button>
              <button onClick={() => setDrawer(null)} className="btn-maroon flex-1 justify-center">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {printDoc && (
        <div className="fixed inset-0 bg-black/40 z-50 grid place-items-center p-4 no-print" onClick={() => setPrintDoc(null)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm">
            <div id="print-area">
              <Receipt title="Pooja Ticket" titleTe="పూజ టికెట్" no={printDoc.ticket_no} subNo={printDoc.booking_code} subNoLabel="Booking No" amount={printDoc.amount}
                rows={[
