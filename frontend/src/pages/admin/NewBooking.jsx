...
import { DevoteesAPI, PoojasAPI, BookingsAPI, PaymentsAPI, PoojarisAPI } from '../../api/client.js'
          ...
          <React.Fragment key={i}>
            <div className={`flex items-center gap-3 px-4 py-3 flex-1 ${banner ? 'bg-maroon-800 text-cream' : ''}`}
              ...
              <div className="hidden md:block">
                <div className={`text-[13px] font-bold ${banner ? 'text-cream' : active ? 'text-maroon-800' : done ? 'text-gray-700' : 'text-gray-400'}`}>{s.t}</div>
                <div className={`text-[11px] ${banner ? 'text-cream/70' : 'text-gray-400'}`}>{sub}</div>
              </div>
            </div>
...
export default function NewBooking() {
  ...
  return (
    <div>
      <div className="text-[12px] text-gray-400 mb-1"><Link to="/admin/bookings" className="hover:text-maroon-600">Pooja Management</Link> › <Link to="/admin/bookings" className="hover:text-maroon-600">Bookings</Link> › <span className="text-gray-500">New Pooja Booking</span></div>
      <div className="flex items-start justify-between gap-4 mb-5">
        <h1 className="font-serif text-[26px] font-bold text-maroon-800">New Pooja Booking</h1>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1.2fr_auto] gap-5 items-start">
              ...
              <div className="hidden lg:flex flex-col items-center justify-center text-[12px] text-gray-400 self-stretch"><div className="flex-1 w-px bg-gray-100" /><span className="py-1">OR</span><div className="flex-1 w-px bg-gray-100" /></div>
              <div>
                ...
                {devotee ? (
                  <div className="flex items-center gap-3 border border-gray-200 rounded-xl px-3.5 py-3 bg-gray-50/50">
                    ...
                    <div className="flex-1"><div className="font-bold text-gray-800">{devotee.name}</div><div className="text-[12px] text-gray-500">{devotee.mobile}</div></div>
                    <button type="button" onClick={() => setDevotee(null)} className="text-gray-400 hover:text-red-600"><X size={16} /></button>
                  </div>
                ) : devResults === null ? (
                  <div className="border border-dashed border-gray-200 rounded-xl px-3.5 py-4 text-[13px] text-gray-400 text-center">Search and select a devotee.</div>
                ) : (
                  <div className="border border-gray-200 rounded-xl divide-y divide-gray-100 max-h-40 overflow-y-auto">
                    {devResults.map((d) => (
                      <button key={d.id} type="button" onClick={() => setDevotee(d)} className="w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-gray-50">
                        <div className="w-8 h-8 rounded-full bg-amber-50 text-amber-700 grid place-items-center text-[12px] font-bold">{d.name[0]}</div>
                        <div className="flex-1 min-w-0"><div className="font-semibold text-gray-800 text-[13px]">{d.name}</div><div className="text-[11px] text-gray-400">{d.code} · {d.mobile}</div></div>
                      </button>
                    ))}
                    {devResults.length === 0 && (
                      <div className="px-3 py-4 text-center">
                        <div className="text-gray-400 text-[13px] mb-2">No devotee found for “{devQ}”.</div>
                        <button type="button" onClick={openQuickAdd} className="inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-maroon-700 border border-maroon-200 rounded-lg px-3 py-1.5 hover:bg-maroon-50"><UserPlus size={14} /> Create devotee “{devQ}”</button>
                      </div>
                    )}
                  </div>
                )}
            ...
            {quickAdd && (
              <div className="mt-4 border border-maroon-200 bg-maroon-50/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2 text-maroon-700"><UserPlus size={16} /><h4 className="font-semibold text-[14px]">Quick Add Devotee</h4></div>
                ...
                {qaErr && <div className="text-[12px] text-red-600 mt-2">{qaErr}</div>}
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className="text-[11.5px] text-gray-400 ml-1">You can complete the full profile later from Devotee Management.</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="mt-4 bg-blue-50/60 border border-blue-100 rounded-lg px-3.5 py-2.5 text-[12.5px] text-gray-600 flex items-start gap-2"><Info size={15} className="text-blue-500 shrink-0 mt-0.5" /> Only plans configured for the selected pooja are shown below.</div>
            </div>
                      ...
                      {pooja.plans.map((pl) => {
                        ...
                        return (
                          <tr key={pl.id} className={on ? 'bg-maroon-50/40' : 'hover:bg-gray-50/60'}>
                            ...
                            <td className="px-2 py-3 text-right font-semibold text-gray-800">{pl.committee_decided ? <span className="text-amber-600 text-[12px]">Committee</span> : money2(pl.fee)}</td>
                            <td className="px-2 py-3 text-gray-600 text-[13px]">{validityShort(pl)}</td>
                            <td className="px-2 py-3"><span className="text-[11px] font-semibold text-emerald-700 bg-emerald-50 rounded-full px-2.5 py-0.5">Available</span></td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr_1fr] gap-5 items-start">
              {plan ? (
                <div className="bg-emerald-50/60 border border-emerald-100 rounded-xl px-4 py-3.5 flex items-center gap-3">
                  ...
                  <div><div className="font-bold text-gray-800">{plan.plan_name} {shortPooja(pooja?.name)}</div><div className="text-[12px] text-gray-500">{plan.plan_name} · {plan.committee_decided ? 'Committee' : inr(plan.fee)} · Validity: {validityShort(plan)}</div></div>
                </div>
              ) : <div className="border border-dashed border-gray-200 rounded-xl px-4 py-4 text-center text-gray-400 text-[13px]">Select a plan first.</div>}
              <div>
                <label className="label">Booking Date *</label>
                <div className="relative"><input type="date" className="input pr-9" value={schedDate} onChange={(e) => setSchedDate(e.target.value)} /><Calendar size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /></div>
                {plan && <div className="text-[12px] text-gray-400 mt-1.5">Booking will be valid for {validityShort(plan)} from the selected date.</div>}
              </div>
              <div>
                <label className="label">Time Slot *</label>
                <div className="relative"><select className="input pr-9 appearance-none" value={slot} onChange={(e) => setSlot(e.target.value)}>{SLOTS.map((s) => <option key={s} value={s}>{s}</option>)}</select><Clock size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /></div>
                <div className="text-[12px] text-gray-400 mt-1.5">Select the pooja timing slot for the booking.</div>
              </div>
              <div>
                <label className="label">Assign Poojari (Optional)</label>
                <div className="relative"><select className="input pr-9 appearance-none" value={poojariId} onChange={(e) => setPoojariId(e.target.value)}><option value="">Not assigned</option>{poojaris.map((p) => <option key={p.id} value={p.id}>{p.name}{p.specialization ? ` · ${p.specialization}` : ''}</option>)}</select><User size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /></div>
                <div className="text-[12px] text-gray-400 mt-1.5">Optionally assign a poojari to perform this booking.</div>
              </div>
            </div>
          </div>
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 lg:grid-cols-[1.7fr_1fr] gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="divide-y divide-gray-100">
                {summaryRows(false).map((r) => { const Icon = r.icon; return (
                  <div key={r.k} className="flex items-center gap-3 py-2.5 text-[13.5px]"><Icon size={15} className="text-gray-400 shrink-0" /><span className="text-gray-500 w-32 shrink-0">{r.k}</span><span className="text-gray-400">:</span><span className="text-gray-800 font-medium">{r.v}</span></div>
                ) })}
              </div>
              <div className="mt-3 bg-blue-50/60 border border-blue-100 rounded-lg px-3.5 py-2.5 text-[12.5px] text-gray-600 flex items-start gap-2"><Info size={15} className="text-blue-500 shrink-0 mt-0.5" /> Please verify all details before proceeding to payment.</div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-1 mb-3">
                {['Cash', 'UPI/QR Code'].map((m) => { const on = method === m; return (
                  <button type="button" key={m} onClick={() => setMethod(m)} className={`flex items-center gap-2.5 border rounded-lg px-4 py-3 ${on ? 'border-maroon-400 bg-maroon-50/40 ring-1 ring-maroon-200' : 'border-gray-200 hover:border-maroon-300'}`}>
                    ...
                    <span className="text-[13px] font-semibold text-gray-700">{modeLabel(m)}</span>
                  </button>
                ) })}
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
                <div>
                  <div className="bg-amber-50/60 border border-amber-100 rounded-lg px-3.5 py-2.5 text-[12px] text-gray-600 flex items-start gap-2 mb-3"><Info size={14} className="text-amber-600 shrink-0 mt-0.5" /> If payment mode is UPI / QR Code, please enter the UTR / Transaction ID.</div>
                ...
                <div>
                  ...
                  <div className="text-[11px] text-gray-400 mt-1">Enter UPI transaction reference number / UTR</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50/50 border border-amber-100 rounded-lg px-4 py-3 text-[12.5px] text-gray-600 flex items-start gap-2"><Info size={15} className="text-amber-600 shrink-0 mt-0.5" /> All payments are subject to temple rules and availability.</div>
          </div>
      ...
      {step === 2 && ticket && (
        <div>
          <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl px-6 py-4 flex items-center gap-4 mb-5">
            ...
            <div><h2 className="font-serif text-xl font-bold text-emerald-700">Booking Successful!</h2><p className="text-[13px] text-gray-500">Your pooja booking has been confirmed successfully. Thank you for your devotion.</p></div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="divide-y divide-gray-100 text-[13.5px]">
              ...
              <div className="mt-3 bg-blue-50/60 border border-blue-100 rounded-lg px-3.5 py-2.5 text-[12.5px] text-gray-600 flex items-start gap-2"><Info size={15} className="text-blue-500 shrink-0 mt-0.5" /> Please show this ticket at the temple counter / pooja venue.</div>
            </div>
            ...
            <div id="print-area">
              <div className="bg-[#fdf7ee] border-2 border-dashed border-amber-300 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-4 border-t border-dashed border-amber-200 pt-4">
                  ...
                  <TField k="Devotee" v={<>{devotee.name}<span className="block text-[11px] text-gray-500 font-normal">{devotee.mobile}</span></>} />
                  ...
                  <div className="bg-amber-100/60 rounded-lg px-3 py-2 col-span-2 flex items-center justify-between"><span className="text-[11px] text-gray-500">Amount Paid (₹)</span><span className="font-extrabold text-maroon-800">{money2(fee)}</span></div>
                  <TField k="Payment Mode" v={modeLabel(ticket._method)} />
                  <TField k="Payment Date & Time" v={ticket._paidAt} />
                </div>
                <div className="text-center mt-4 pt-3 border-t border-dashed border-amber-200"><div className="font-display text-maroon-700 tracking-wide text-sm">✦ Om Sai Ram ✦</div><div className="text-[11px] text-gray-500 mt-0.5">Thank you for your devotion. May Sai Baba bless you.</div></div>
              </div>
            </div>
          </div>
...
function TField({ k, v, sub, mono }) {
  return <div><div className="text-[11px] text-gray-500">{k}</div><div className={`text-[13px] text-gray-800 font-semibold ${mono ? 'font-mono' : ''}`}>{v}</div>{sub && <div className="text-[10px] text-gray-400">{sub}</div>}</div>
}
